...
It will later come in handy to transfer the complex equation to polar coordinates by changing the base from $x,y$ to $r,\phi$. After carrying out this transformation, trigonometric expressions for the solution function can be described. As $\hat{f}$ is a complex function, it has a real and imaginary part:  
...
$$C²=[cos(\theta_a)+cos(\theta_b)]²+[sin(\theta_a)+sin(\theta_b)]²$$  
